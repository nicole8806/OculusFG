...
void OculusViewer::configure()
{
	...
	m_view->addSlave(m_cameraRTTLeft.get(),
		...
		true);
	m_view->getSlave(0)._updateSlaveCallback = new OculusUpdateSlaveCallback(OculusUpdateSlaveCallback::LEFT_CAMERA, m_device.get(), m_swapCallback.get(), m_warning.get(),m_cameraRTTLeft);

	m_view->addSlave(m_cameraRTTRight.get(),
		m_device->projectionOffsetMatrixRight(),
		m_device->viewMatrixRight(),
		true);
	m_view->getSlave(1)._updateSlaveCallback = new OculusUpdateSlaveCallback(OculusUpdateSlaveCallback::RIGHT_CAMERA, m_device.get(), m_swapCallback.get(), 0,m_cameraRTTRight);
